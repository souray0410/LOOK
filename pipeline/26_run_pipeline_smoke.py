...
def main() -> None:
    ...
    zero_result = run_arm("raw_zero", paths, torch, gpu_devices)
    mean_result = run_arm("normalized_mean", paths, torch, gpu_devices)
    gan_result = run_arm("paired_cgan", paths, torch, gpu_devices)
    assert zero_result["status"] == mean_result["status"] == gan_result["status"] == "complete"
    assert zero_result["checkpoint"]["backbone_id"] == mean_result["checkpoint"]["backbone_id"]
    ...
    print("PIPELINE SMOKE PASS: shared frozen classifier, three fills, LOOK, sealed test")
