...
with (B/'manager.lock').open('a') as lock:
 ...
 dependencies(S);assert S['spatial_factors']==[16] and S['mode']=='best_forward' and S['host']['seed']==3416
 ...
 try:
  for phase,out in [('profile',profile),('formal',R)]:
   ...
   while child.poll() is None:
    ...
    if phase=='formal' and not released_old and list((R/'corrections').glob('*/factor_*/**/site_cache/*/accepted.json')):
     write(dict(reason='fixed16_replacement_verified',replacement=str(R),time=time.time()),pathlib.Path(C['superseded_search'])/'pause.json');released_old=True
  ...
  verify_case(R,S);claims.release(R,owner,'completed',step_dead=True)
