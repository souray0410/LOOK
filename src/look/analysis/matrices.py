...
def _plot_matrix_bank(records: Sequence[Dict[str, object]], output_dir: Path) -> None:
    ...
    axes[0, 1].plot(x, [record["applied_latent_map"]["spectral_norm"] for record in records], marker="s", label="I + W")
...
def analyze_correction_matrix(
    ...
    """Describe W in delta-z = zW+b and the applied residual map I+W."""
    ...
    corrected_map = np.eye(dimension) + weight
    corrected_singular_values = np.linalg.svd(corrected_map, compute_uv=False)
    corrected_eigenvalues = 1.0 + eigenvalues
    ...
    record: Dict[str, object] = {
        ...
        "matrix_semantics": "delta_z = z_missing @ W + b; z_corrected = z_missing + delta_z",
        ...
        "applied_bias_l2": float(np.linalg.norm(bias)),
