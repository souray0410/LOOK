...
def report(root):
    ...
    text=['# LOOK 单配置全流程交付','',f"配置：{spec['host']}；策略 {spec['mode']}；空间因子 {spec.get('spatial_factors')}；潜在维度 {spec.get('latent_dims')}。",
        ...
        ('每个已接受前缀独立评价全部下游位置，各位置只保留自身最佳候选，严格正增益扩展全部成分支；无正增益扩展不产生后代。最终报告开发集分数最高路径，全部路径及拒绝证据保留。' if spec['mode']=='positive_forward_tree' else '先扫描剩余下游位置，选择最大正收益；后续拟合使用此前已接受修正。无正收益保留当前模型并停止。' if spec['mode']=='best_forward' else '从指定起点按顺序逐级拟合；每级仅接受正收益修正，否则跳过该级继续。后续拟合使用此前已接受修正。'),'',
