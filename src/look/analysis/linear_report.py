...
def report(records,out):
    ...
    for record in records:
        ...
        if file_sha256(path)!=record['sha256']:raise ValueError('Changed linear predictions')
        with np.load(path,allow_pickle=False) as f:a={k:f[k] for k in f.files}
        key=(record['method'],record['scenario'])
        if key in arrays:raise ValueError('Duplicate linear view')
        arrays[key]=a
    ...
    if not required.issubset(arrays):raise ValueError('Incomplete three-arm comparison')
    ...
    for a in arrays.values():
        if not np.array_equal(a['participant_ids'],ref['participant_ids']) or not np.array_equal(a['labels'],ref['labels']):
            raise ValueError('Unpaired linear evidence')
    keys=list(arrays);contrasts=[];definitions=[]
    for first,second in protocol()['comparisons']:
...
def group_report(runs,out):
    ...
    out=Path(out);out.mkdir(parents=True,exist_ok=True);seen=set();group_key=None;keys=[];pred=[];rows=[];reference=None;receipts={}
    for run in runs:
        root=Path(run);spec=read(root/'spec.json');verify_case(root,spec);seed=spec['host']['seed']
        current=tuple(spec['host'][k] for k in ('disease','architecture','position'))
        if group_key is not None and current!=group_key:raise ValueError('Unmatched host group')
        group_key=current
        if seed in seen:raise ValueError('Duplicate seed in linear group')
