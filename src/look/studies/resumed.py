...
from look.runtime.scheduler import read, record, verify, atomic, digest, check_source, supervise, validate_receipt
...
def verify_previous(case, row):
    from look.studies.starts import audit_case, verify_file
...
def prepare(root, config_path, output, acceptance):
    from look.studies.starts import verify_source, audit_case, choose_start, PATTERNS
    ...
    for rec in inventory.get('protected_sources',[]):
        from look.studies.starts import verify_file
        verify_file(rec)
...
def collect(output):
    from look.studies.starts import audit_case
