"""One immutable suffix fit; shares frozen host/PCA without rewriting either."""
import argparse
import fcntl
import os
...
from look.studies.suffix_protocol import VERSION, PATTERNS, validate
from look.methods.operator import FullFeaturePCA, greedy_fit_look, load_selected_bank, forward_with_look
...
def check_files(root, files):
    ...
    for name,sha in files.items():
        p=(root/name).resolve()
        if not p.is_relative_to(root) or file_sha256(p)!=sha:raise ValueError('Suffix evidence changed')
...
def verify_case(root,spec):
    ...
    if (r.get('schema')!=VERSION or r.get('identity')!=stable_hash(spec) or r.get('state')!='accepted'
        ...
        or not all(r.get(k) is True for k in ('host_frozen','reload_exact','rng_restored','prefix_disabled','pca_unchanged'))):
        raise ValueError('Suffix has not passed technical acceptance')
    ...
    if not required.issubset(r['files']):raise ValueError('Missing suffix evidence')
...
def dependencies(spec):
    ...
    for p,sha in ((s['spec_path'],s['spec_sha256']),(root/'host/accepted.json',s['host_accepted_sha256']),
        ...
        if file_sha256(p)!=sha:raise ValueError('Suffix dependency changed')
    ...
    if spec.get('pilot'):
        ...
        p=spec['pilot']
        if file_sha256(Path(p['path'])/'accepted.json')!=p['sha256']:raise ValueError('Pilot changed')
        r=verify_group(p['path'])
        if r['host']!=dict(h,seed=3416):raise ValueError('Different pilot host')
    return base,root,m
...
def load_basis(spec,manifest):
    ...
    for e in manifest['entries']:
        ...
        bank[key]=b
    if {k[0] for k in bank}!=allowed:raise ValueError('Incomplete suffix PCA entries')
    return bank
...
def run(spec,out,device,pause,profile=False):
    ...
    if not profile:
        p=Path(os.environ['LOOK_SUFFIX_PROFILE_RECEIPT']);r=read(p)
        if r.get('identity')!=stable_hash(spec) or r.get('state')!='accepted' or r.get('profile') is not True:
            raise ValueError('Suffix resource receipt mismatch')
    ...
    try:
        ...
        for pattern in PATTERNS:
            ...
            atomic_write_json(dict(state='running',stage='suffix_fit',pattern=pattern,start=spec['start_ordinal'],updated_at=time.time()),out/'status.json')
            if (target/'factor_selection.json').exists():artifacts=load_selected_bank(target)
            else:
                artifacts,_=greedy_fit_look(graph,train_loader,dev_loader,pattern,spec['eligible_sites'],
            ...
            for name,result in [('suffix',corrected),('host',baseline)]:
        ...
        costs=dict(seconds=time.time()-started,gpu_peak_reserved_bytes=torch.cuda.max_memory_reserved(device),
    ...
    receipt=dict(schema=VERSION,identity=stable_hash(spec),state='accepted',profile=profile,test_access=False,
        host_frozen=True,reload_exact=True,rng_restored=True,prefix_disabled=True,pca_unchanged=True,files=files)
