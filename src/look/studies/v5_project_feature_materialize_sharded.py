...
def materialize(*,source_run,checkpoint,output,inputs_factory,device,gpu_budget_bytes,max_batches=None):
 ...
 with torch.no_grad():
  for index,batch in enumerate(loader):
   final=chunks/f'{index:06d}';partial=chunks/f'{index:06d}.partial';meta=final/'receipt.json'
   if final.exists():
    row=read(meta)
    if row['batch']!=index or row['participants_sha256']!=participant_sha(batch['participant_id']) or row['sites']!=sites: raise ValueError('Existing feature chunk changed')
