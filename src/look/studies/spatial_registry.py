...
from look.studies.spatial_protocol import VERSION,matrix,protocol
...
def tick(config):
    ...
    with (out/'registry.lock').open('a') as lock:
        ...
        if path.exists() and read(path)!=p:raise ValueError('Spatial protocol changed')
        ...
        for h in sorted(matrix(),key=lambda h:(h['seed'],h['disease'],h['architecture'],h['position'])):
            ...
            if h['seed']!=3416 and group not in pilots:wait[name]='waiting_complete_three_route_pilot';continue
            ...
            if specpath.exists() and read(specpath)!=spec:raise ValueError('Spatial task identity changed')
            atomic_write_json(spec,specpath)
            run=Path(reserve(out,VERSION,stable_hash(h),spec,source=hosts[key],refresh_summary=False))
            row=dict(id=name,spec=str(specpath),spec_sha256=file_sha256(specpath),run_dir=str(run),execution='look_spatial')
        ...
        for key,runs in report_groups.items():
            ...
            from look.runtime.spatial_analysis import request
            request(config,runs,target)
        result=dict(schema='look_spatial_work_feed_v1',test_access=False,tasks=ready,expected_hosts=81,expected_route_views=243,
