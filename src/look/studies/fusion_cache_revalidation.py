...
def revalidate_or_fit(*,source,target,graph,loader,device,arm,pattern,sites,factor,pca_bank,identity,
    ...
    receipt=dict(schema='look_fusion_fresh_revalidation_v2',state='accepted',identity=identity,test_access=False,
        ...
        relocation=dict(
            pre_fit=dict(receipt=str(relocation_pre_path),sha256=file_sha256(relocation_pre_path),
                mapping_count=relocation_pre['mapping_count'],verified_target_refs=relocation_pre['verified_target_refs']),
            post_fit=dict(receipt=str(relocation_post_path),sha256=file_sha256(relocation_post_path),
                mapping_count=relocation_post['mapping_count'],verified_target_refs=relocation_post['verified_target_refs'])),
        relocation_no_scientific_value_change=(
            relocation_pre['no_scientific_value_change'] is True and relocation_post['no_scientific_value_change'] is True),
