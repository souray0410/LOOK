...
from look.runtime.state import atomic_write_json, stable_hash, PipelineState, utc_now, file_sha256
from look.studies.starts import read_json, file_record, verify_file, verify_source, pin_source, source_result, verify_parent, SuffixRunner, sites_for_fusion, PATTERNS
from look.runtime.provenance import implementation_sha256, seed_everything
from look.methods.operator import load_selected_bank
from look.methods.imputation import NormalizedMeanFiller
from look.data.dataset import make_loader
from look.evaluation.evaluator import save_prediction_bundle
from look.methods.kernels import fit_control, evaluate_control, forward_control
from look.methods.ssf import SSFAdapter, train_ssf
from look.analysis.methods import representation_evidence, measure_inference
...
def run_method_case(source, case, spec, output, device, devices):
    if case['method']=='logit_affine':
        from look.methods.logit import run_logit_case
    ...
    identity=dict(case=case,spec=spec,source_sha256=stable_hash(source),
                  implementation_sha256=implementation_sha256(Path(__file__).resolve().parents[3]))
...
def run_method_study(paths,parent_path,parent_source,suffix_path,devices,*,execute=False,wait=False,render=True,start_evidence_manifest=None):
    ...
    with PipelineState(output/'state','method_evidence',identity,[spec_path]) as state:
        ...
        try:
            ...
            if render:
                from look.analysis.method_report import write_method_report
            ...
            if inventory_path.exists():
                ...
                for folder in ('src/look','third_party/MHD_Framework/src/mhd_framework'):
                ...
                inventory=dict(sources=sources,upstream_records=[file_record(parent_path),file_record(suffix_path)],
                    scope=spec,source_files=[file_record(p) for p in (paths.project_root/'src/look').glob('method_*.py')])
