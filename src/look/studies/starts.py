...
from look.methods.imputation import NormalizedMeanFiller, RawZeroFiller, PairedCGANFiller
from look.methods.generation import load_generator
from look.methods.joint import correction_sites, PROTOCOL as JOINT_PROTOCOL
from look.methods.operator import FullFeaturePCA, load_selected_bank, validate_global_factor_bank
from look.studies.pipeline import ExperimentRunner, PipelineOptions
from look.runtime.provenance import backbone_implementation_sha256, implementation_sha256, seed_everything
from look.runtime.state import PipelineState, atomic_write_json, file_sha256, stable_hash, utc_now
from look.studies.protocol import correction_stages, validate_spec
...
def pin_source(result_path, parent, runs, project_root):
    ...
    expected = dict(backbone_id=cp['backbone_id'], checkpoint_sha256=cp['sha256'],
        ...
        joint_code_sha256=file_sha256(project_root/'src/look/methods/joint.py'),
        pca_code_sha256=file_sha256(project_root/'src/look/methods/operator.py'),
    ...
    with PipelineState(output/'state', 'suffix_study', identity, [spec_path]) as state:
        ...
        try:
            ...
            if inventory_path.exists():
                ...
                protected = [p for folder in ('src/look','third_party/MHD_Framework/src/mhd_framework') for p in (parent_source/folder).glob('*.py')
            ...
            if render:
                from look.analysis.start_report import write_report
