"""Automatic real work dispatch for selected native replications and LOOK cases.
...
def work(config):
    ...
    for path in config.get('linear_feeds',[]):
        linear=read(path)
        if not linear:continue
        if linear.get('schema')!='look_linear_work_feed_v1' or linear.get('test_access') is not False:
            raise ValueError('Unsealed linear feed')
        result.extend(dict(t,execution='look_linear') for t in linear['tasks'])
    for path in config.get('terminal_feeds',[]):
        stage=read(path)
    ...
    for path in config.get('family_search_feeds',[]):
        stage=read(path)
        if not stage:continue
    ...
    for path in config.get('suffix_feeds',[]):
        stage=read(path)
        if not stage:continue
        if stage.get('schema')!='look_suffix_work_feed_v1' or stage.get('test_access') is not False:
            raise ValueError('Unsealed suffix feed')
        result.extend(dict(t,execution='look_suffix') for t in stage['tasks'])
    for path in config.get('affine_feeds',[]):
        stage=read(path)
        if not stage:continue
        if stage.get('schema')!='look_affine_work_feed_v1' or stage.get('test_access') is not False:
            raise ValueError('Unsealed affine feed')
        for row in stage['tasks']:
            if row.get('scope') not in ('terminal','progressive'):raise ValueError('Unknown affine scope')
            result.append(dict(row,execution='look_affine_'+row['scope']))
    ...
    if (Path(task['run_dir'])/'accepted.json').exists():
        ...
        if task['execution']=='native':
            ...
            if task['execution'] in ('look_affine_terminal','look_affine_progressive'):
                ...
                from look.studies.terminal_case import verify_case
            elif task['execution']=='look_linear':
                from look.studies.linear_case import verify_case
            elif task['execution']=='look_spatial':
...
def admissible_work(config, claims, reservation=None):
    ...
    for task in candidates:
        if task.get('execution')=='look_search' and task.get('search_mode') in config.get('deferred_search_modes',[]):
            rejected.append(dict(run=task['run_dir'],reason='explicit_scientific_priority_amendment',search_mode=task['search_mode']))
            continue
...
def verify_delivery_dependency(entry):
    ...
    modules={'look':'project_case','look_family_search':'family_search_case','look_search':'search_case','look_suffix':'suffix_case',
...
def reconcile_expired(config):
    ...
    for task in work(config):
        ...
        if not str(record.get('owner','')).startswith('look-workflow-'):continue
...
def gpu_owner(config_path):
    ...
    while time.time()<end-1800:
        ...
        if reservation:task=reservation[0]
            ...
            supplement.sort(key=lambda t: -6 if t['execution']=='look_family_search' else -5 if t['execution']=='look_search' and t.get('search_mode')=='greedy' and search_counts['best_forward']>0 else -4 if t['execution']=='look_search' and t.get('search_mode')=='best_forward' else -3 if t['execution']=='look_search' else -2 if t['execution'] in ('look_terminal','look_suffix','look_search','look_family_search','look_affine_terminal') else -1 if t['execution'] in ('look_spatial','look_linear','look_affine_progressive') else priority[read(t['spec'])['task']['kind']])
            task=supplement[0]
        ...
        if state=='completed' or (state=='infeasible' and task['execution']=='look_mechanism'):
            ...
            if task['execution']=='look_family_search':
                from look.analysis.family_search_report import report as family_report
                family_report(run)
                if config.get('family_search_package_feed'):
                    from look.analysis.family_search_report import refresh_package
                    refresh_package(config['family_search_package_feed'],config['family_search_package_output'],
                        arms=tuple(config['family_search_package_arms']))
...
def execute_work(config_path,spec_path,run,kind,record):
    ...
    if kind in ('look_mechanism','look_spatial','look_linear','look_terminal','look_suffix','look_search','look_family_search','look_affine_terminal','look_affine_progressive'):
        ...
        if kind not in ('look_search','look_family_search') and Path(look.__file__).resolve().parent!=Path(source)/'look':
    ...
    if kind=='native':
        ...
        subprocess.run([config['python'],'-m','look.studies.affine_case','--spec',str(spec_path),
        ...
        receipt_key='LOOK_FAMILY_SEARCH_PROFILE_RECEIPT' if kind=='look_family_search' else 'LOOK_SEARCH_PROFILE_RECEIPT'
        ...
        try:
            run_profile([config['python'],'-m',case_module,'--spec',str(spec_path),
        ...
        environment[receipt_key]=str(receipt_path)
        os.execvpe(config['python'],[config['python'],'-m',case_module,
            '--spec',str(spec_path),'--output',str(run)],environment)
    elif kind=='look_suffix':
        environment['LOOK_WORKER_MEMORY_BYTES']=str(100*1024**3)
        subprocess.run([config['python'],'-m','look.studies.suffix_case','--spec',str(spec_path),
            '--output',str(profile_root),'--profile'],env=environment,check=True)
        receipt_path=profile_root/'profile_accepted.json';receipt=read(receipt_path)
        if receipt.get('state')!='accepted' or receipt.get('identity')!=stable_hash(spec):
            raise ValueError('Suffix resource preflight failed')
        environment['LOOK_SUFFIX_PROFILE_RECEIPT']=str(receipt_path)
        os.execvpe(config['python'],[config['python'],'-m','look.studies.suffix_case',
            '--spec',str(spec_path),'--output',str(run)],environment)
    elif kind=='look_terminal':
        environment['LOOK_WORKER_MEMORY_BYTES']=str(100*1024**3)
        subprocess.run([config['python'],'-m','look.studies.terminal_case','--spec',str(spec_path),
            '--output',str(profile_root),'--profile'],env=environment,check=True)
        receipt_path=profile_root/'profile_accepted.json';receipt=read(receipt_path)
        if receipt.get('state')!='accepted' or receipt.get('identity')!=stable_hash(spec):
            raise ValueError('Terminal resource preflight failed')
        environment['LOOK_TERMINAL_PROFILE_RECEIPT']=str(receipt_path)
        os.execvpe(config['python'],[config['python'],'-m','look.studies.terminal_case',
            '--spec',str(spec_path),'--output',str(run)],environment)
    elif kind=='look_linear':
        environment['LOOK_WORKER_MEMORY_BYTES']=str(384*1024**3)
        subprocess.run([config['python'],'-m','look.studies.linear_case','--spec',str(spec_path),
            '--output',str(profile_root),'--profile'],env=environment,check=True)
        receipt_path=profile_root/'profile_accepted.json';receipt=read(receipt_path)
        if receipt.get('state')!='accepted' or receipt.get('identity')!=stable_hash(spec):
            raise ValueError('Linear full-path resource receipt mismatch')
        environment['LOOK_LINEAR_PROFILE_RECEIPT']=str(receipt_path)
        os.execvpe(config['python'],[config['python'],'-m','look.studies.linear_case',
            '--spec',str(spec_path),'--output',str(run)],environment)
