...
def worker(project_root, plan_path, output, job_id):
    ...
    with (out/'worker.lock').open('a') as lock:
        ...
        import torch
        from look.runtime.bounded import install_runtime, BudgetRunner
        ...
        begun = time.time()
        if job['kind'] == 'acceptance':
            from look.runtime.scheduler_checks import probe
            result_path = probe(source, out, job['seed'])
        elif job['kind'] == 'method':
            from look.studies.methods import make_method_cases, run_method_case
            ...
            from look.studies.starts import audit_case, evaluate_selected
            rows = []
