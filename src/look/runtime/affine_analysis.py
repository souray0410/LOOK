"""Finite CPU Slurm analysis, invoked under the affine registry lock."""
import argparse
import json
...
def request(config,runs,out):
    ...
    script.write_text('#!/bin/bash\nset -euo pipefail\nexport OMP_NUM_THREADS=8 MKL_NUM_THREADS=8 OPENBLAS_NUM_THREADS=8\n'+
        ...
        shlex.join([sys.executable,'-m','look.runtime.affine_analysis','--manifest',str(manifest)])+'\n')
    ...
    command=['sbatch','--parsable','--account=pi-mengy','--nodes=1','--ntasks=1','--cpus-per-task=8',
        '--mem=64G','--time=48:00:00','--job-name=look_affine_statistics','--output='+str(out/'analysis_%j.log'),str(script)]
    try:
...
def main():
    ...
    cases=[]
    for run in m['runs']:
        root=Path(run);spec=json.loads((root/'spec.json').read_text());verify_case(root,spec)
        records=json.loads((root/'development/suite.json').read_text())['records']
        cases.append((spec['host'],spec['scope'],records))
    report(cases,m['output'])
