"""Finite CPU Slurm analysis, invoked under the spatial registry lock."""
import argparse
import json
...
def request(config,runs,out):
    ...
    script.write_text('#!/bin/bash\nset -euo pipefail\nmodule load python/3.11\nexport OMP_NUM_THREADS=8 MKL_NUM_THREADS=8 OPENBLAS_NUM_THREADS=8\n'+
        ...
        shlex.join([sys.executable,'-m','look.runtime.spatial_analysis','--manifest',str(manifest)])+'\n')
    ...
    command=['sbatch','--parsable','--account=pi-mengy','--nodes=1','--ntasks=1','--cpus-per-task=8',
        '--mem=64G','--time=48:00:00','--job-name=look_spatial_statistics','--output='+str(out/'analysis_%j.log'),str(script)]
    try:
...
def main():
    ...
    from look.analysis.spatial_report import group_report
