...
from look.training.distributed import assert_module_state_identical

from look.models.graph import classification_loss_metadata, optimizer_parameter_groups, reset_and_forward
from look.evaluation.metrics import validation_binary_auroc, validation_macro_f1
from look.evaluation.stability import logit_metrics
from look.runtime.monitoring import TrainingMonitor
from look.runtime.provenance import backbone_implementation_sha256, sha256, write_json_atomic
...
def train_complete_model_ddp(
    ...
    backbone_hash = backbone_implementation_sha256(Path(__file__).resolve().parents[3])
    ...
    monitor = TrainingMonitor(run_dir, tuple(config.class_names)) if context.is_main else None
