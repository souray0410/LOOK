...
from look.methods.joint import members, member_shapes, read_site, write_site, site_level
from look.methods.operator import PROTOCOL, LOOKArtifact, LatentSufficientStatistics, _gcv_lambda, _reset_inputs, apply_artifact, downsample_flatten, save_selected_bank, load_selected_bank
    ...
    for level in range(-1, site_level(graph, target) + 1):
        ...
        for a in artifacts:
            ...
            if policy == 'self_input_missing_only':
                from look.methods.self_input import apply_self_input
...
def fit_candidates(graph, loader, node, pattern, factor, dims, max_rank, device,
    ...
    for batch in loader:
        ...
        if policy == 'self_input_missing_only' and node.startswith('joint_'):
            from look.methods.self_input import neutralize_retained
