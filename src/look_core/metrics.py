...
@torch.no_grad()
def validation_binary_auroc(graph) -> torch.Tensor:
    """Exact binary AUROC from complete validation logits using the Mann-Whitney form."""
    logits = graph.get_node_by_name("fusion_logits").feature_message.current_state
    labels = graph.get_node_by_name("label_gt").feature_message.current_state.long()
    ...
    if not positive.numel() or not negative.numel():
        raise ValueError("validation_binary_auroc requires both classes")
    comparisons = positive[:, None] - negative[None, :]
    return (comparisons.gt(0).float() + 0.5 * comparisons.eq(0).float()).mean()
...
def classification_metrics(y_true: np.ndarray, probabilities: np.ndarray) -> Dict[str, object]:
    ...
    for class_id in range(num_classes):
        try:
            auroc_per_class.append(
                float(roc_auc_score(one_hot[:, class_id], probabilities[:, class_id]))
            )
        except ValueError:
        ...
        try:
            auprc_per_class.append(
                float(
                    average_precision_score(
                        one_hot[:, class_id], probabilities[:, class_id]
                    )
                )
            )
        except ValueError:
    ...
    try:
        if num_classes == 2:
            macro_auc = float(roc_auc_score(y_true, probabilities[:, 1]))
        else:
            macro_auc = float(
    ...
    return {
        ...
        "macro_auprc_ovr": float(np.nanmean(auprc_per_class)),
