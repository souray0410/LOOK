...
def source_hashes(project_root: Path, extra_paths: Iterable[Path] = ()) -> Dict[str, str]:
    ...
    paths.extend(sorted((project_root / "src/look_core").glob("*.py")))
    paths.extend(sorted((project_root / "scripts").glob("*")))
    paths.extend(sorted((project_root / "third_party/MHD_Project/V4").glob("*.py")))
...
def implementation_sha256(project_root: Path) -> str:
    paths = sorted((project_root / "src/look_core").glob("*.py"))
    paths.extend(sorted((project_root / "third_party/MHD_Project/V4").glob("*.py")))
    digest = hashlib.sha256()
    for path in paths:
...
def backbone_implementation_sha256(project_root: Path) -> str:
    ...
    relative_paths = (
        "src/look_core/data.py",
        "src/look_core/distributed.py",
        "src/look_core/distributed_worker.py",
        "src/look_core/graph.py",
        "src/look_core/metrics.py",
        "src/look_core/monitoring.py",
        "src/look_core/train.py",
    )
