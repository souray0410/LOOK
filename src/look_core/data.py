...
def validate_reference_table(labels_csv: Path, data_root: Path, check_paths: bool = False) -> Dict[str, object]:
    ...
    if set(observed) != {0, 1} or observed[0] != "normal" or not observed[1]:
        raise ValueError(f"Expected a normal-versus-positive binary mapping, found: {observed}")
