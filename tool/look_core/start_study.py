...
def pin_source(result_path, parent, runs, project_root):
    ...
    expected = dict(backbone_id=cp['backbone_id'], checkpoint_sha256=cp['sha256'],
        ...
        joint_code_sha256=file_sha256(project_root/'tool/look_core/joint.py'),
        pca_code_sha256=file_sha256(project_root/'tool/look_core/look.py'),
    ...
    with PipelineState(output/'state', 'suffix_study', identity, [spec_path]) as state:
        ...
        try:
            ...
            if inventory_path.exists():
                ...
                protected = [p for folder in ('tool/look_core','third_party/MHD_Project/V4') for p in (parent_source/folder).glob('*.py')
