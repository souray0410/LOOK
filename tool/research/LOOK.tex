    ...
    \column{0.54\textwidth}
      \begin{align*}
        ...
        \widehat{x}_{\mathrm{corr}} &= x_{\mathrm{miss}}
          + U\!\left(P^{\top}\Delta z\right).
      \end{align*}
      \footnotesize
      $D/U$: spatial down/up-sampling; $\mathcal{N}$: stored feature normalization;
      $P$: PCA projection; $W,b$: ridge correction; the decoded residual is injected directly.
...
\begin{frame}{How the correction is estimated}
  ...
  \begin{tikzpicture}[node distance=2.5mm,font=\footnotesize]
    ...
    \node[dec,text width=1.9cm,right=of ridge] (inject) {Decode and\\inject residual};
...
\begin{frame}{Preliminary validation: a positive signal, not a final claim}
  ...
  \begin{alertblock}{Required next step}
    ...
    correction state and seed, then rerun validation and untouched test evaluation.
  \end{alertblock}
\end{frame}
