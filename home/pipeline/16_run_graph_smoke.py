...
def main() -> None:
    ...
    for position in ("oct_only", "cfp_only"):
        graph = build_resnet50_mhd_graph(
            position, num_classes=4, batch_size=2, device=device, pretrained=False
        )
        with torch.no_grad():
            logits = reset_and_forward(
                graph, batch["oct"].to(device), batch["cfp"].to(device)
        ...
        del graph
        torch.cuda.empty_cache()
    print(f"SMOKE PASS: seven fusion and two unimodal topologies, GPUs={gpu_devices}")
