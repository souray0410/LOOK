#!/usr/bin/env python3
"""Initialize the portable LOOK 1.0.0 runtime layout and source manifest."""
...
def source_manifest(project_root: Path, release_version: str) -> Path:
...
def release_manifest(project_root: Path, data_root: Path, source_manifest_path: Path, release_version: str) -> Path | None:
...
def main() -> int:
    ...
    manifest = source_manifest(project_root, config["release_version"])
    if args.source_only:
        print(manifest)
        return 0
    data_root = Path(
    ...
    dataset_root = Path(
        args.dataset_root or os.environ.get("LOOK_DATASET_ROOT") or data_root / directories["dataset"]
    ).expanduser().resolve()
    cache_root = Path(
    ...
    generated_release = release_manifest(project_root, data_root, manifest, config["release_version"])
