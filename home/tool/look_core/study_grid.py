...
def _aggregate_confirmation(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for fusion_position, candidates in grouped.items():
        ...
        for metric in ("macro_f1", "balanced_accuracy", "macro_auroc_ovr", "ece_15"):
    ...
    result.sort(key=lambda row: (
        -row["mean_macro_f1"], -row["mean_balanced_accuracy"],
        -row["mean_macro_auroc_ovr"], row["mean_ece_15"], row["std_macro_f1"],
    ))
...
def run_baseline_selection(
    ...
    *,
    ...
    identity = {
        "protocol": "balanced_four_class_baseline_selection",
...
def study_grid_from_baseline_selection(
    ...
    if payload.get("protocol") != "balanced_four_class_baseline_selection":
    ...
    return StudyGrid(
        ...
        filling_strategies=list(filling_strategies or ["normalized_mean", "paired_cgan"]),
