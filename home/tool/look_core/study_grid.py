...
class StudyGrid:
    ...
    def validate(self) -> None:
        if self.backbones != ["resnet50"]:
            raise ValueError("LOOK 1.0.0 supports only the resnet50 backbone")
...
def expand_study_grid(
    ...
    *,
    ...
    if phase == "test" and confirmation != "CONFIGURATION_FROZEN":
        raise ValueError("Test execution requires CONFIGURATION_FROZEN")
    ...
    for backbone, fusion, seed, filling, classifier, look_profile in axes:
        ...
        for gan in gan_variants:
            ...
            options = PipelineOptions(
                ...
                allow_test=phase == "test",
                frozen_test_confirmation=confirmation,
                resume=resume,
                restart=restart,
        ...
        completed.append(
            {
                ...
                "result_file": str(runner.experiment_dir / "experiment_result.json"),
