...
def enforce_single_gpu() -> None:
    visible = os.environ.get("CUDA_VISIBLE_DEVICES")
    if not visible or "," in visible:
        raise RuntimeError(
            "Set CUDA_VISIBLE_DEVICES to one physical GPU index before importing torch."
        )
    if torch.cuda.is_available() and torch.cuda.device_count() != 1:
        raise RuntimeError("Exactly one CUDA device must be visible to this process.")
...
def environment_manifest(
    ...
    data_contract_paths = [
        ...
        labels_csv.parent / "cohort_flow.json",
        labels_csv.parent / "cohort_exclusions.csv.gz",
        labels_csv.parent / "verification.json",
    ]
    return {
        "config": config,
