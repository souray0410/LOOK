...
def _make_node(node_id: int, name: str, stage: str, batch_size: int, device: torch.device, num_classes: int) -> MHD_Node:
    ...
    return MHD_Node(node_id, name, state, transfer_mode="replace")


def _add_edge(edges: List[MHD_Edge], name: str, operation: nn.Module) -> None:
    edges.append(MHD_Edge(len(edges), name, [operation]))
...
def build_resnet50_mhd_graph(
    ...
    nodes.append(MHD_Node(len(nodes), "label_gt", torch.zeros(batch_size, dtype=torch.long, device=device)))
    nodes.append(MHD_Node(len(nodes), "loss", torch.zeros((), device=device)))
    nodes.append(MHD_Node(len(nodes), "batch_accuracy", torch.zeros((), device=device)))
...
def reset_and_forward(
    ...
    batch_size = oct_tensor.shape[0]
    graph.update_batch_size(batch_size)
    ...
    logits = graph.get_node_by_name("fusion_logits").current_state
    if labels is None:
        return logits
    graph.get_node_by_name("label_gt").current_state = labels.long()
    graph.forward(levels=graph.monitor_levels)
    return {
        "fusion_logits": logits,
        "loss": graph.get_node_by_name("loss").current_state,
        "batch_accuracy": graph.get_node_by_name("batch_accuracy").current_state,
    }
    ...
    for edge in graph.edges:
        ...
        for operation in edge.edge_operations:
            if isinstance(operation, nn.Module):
                target.extend(operation.parameters())
    return [
