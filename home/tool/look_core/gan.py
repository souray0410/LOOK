...
from MHD_Project.MHD_Utils_V3 import MHDDistributedContext, mhd_barrier
...
def train_paired_cgan_direction(
    ...
    for epoch in range(start_epoch, config.gan_epochs):
        ...
        discriminator.train()
        sum_g, sum_d = 0.0, 0.0
        for batch in tqdm(train_loader, desc=f"cGAN {source_key}->{target_key} {epoch + 1}", leave=False):
            source = _to_gan_range(batch[source_key].to(device, non_blocking=True))
            target = _to_gan_range(batch[target_key].to(device, non_blocking=True))
            generated = generator(source)
...
def train_paired_cgan_direction_ddp(
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    generator_raw = PairedUNetGenerator(config.gan_base_channels).to(context.device)
    discriminator_raw = PatchDiscriminator(config.gan_base_channels).to(context.device)
    ...
    if last_path.is_file():
        ...
        stale = int(last["stale"])
        history = list(last["history"])
    generator = DistributedDataParallel(generator_raw, device_ids=[context.local_rank]) if context.distributed else generator_raw
    discriminator = DistributedDataParallel(discriminator_raw, device_ids=[context.local_rank]) if context.distributed else discriminator_raw
    ...
    for epoch in range(start_epoch, config.gan_epochs):
        ...
        for batch in tqdm(train_loader, desc=f"cGAN {source_key}->{target_key} {epoch + 1}", leave=False, disable=not context.is_main):
            source = _to_gan_range(batch[source_key].to(context.device, non_blocking=True))
            target = _to_gan_range(batch[target_key].to(context.device, non_blocking=True))
            generated = generator(source)
            optimizer_d.zero_grad(set_to_none=True)
