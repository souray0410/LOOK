...
def initialize_mhd_distributed(backend: Optional[str] = None) -> MHDDistributedContext:
    ...
    return MHDDistributedContext(rank, local_rank, world_size, device, selected_backend)
...
def mhd_barrier(context: MHDDistributedContext) -> None:
    if context.distributed:
        dist.barrier()


def mhd_all_reduce_mean(value: torch.Tensor, context: MHDDistributedContext) -> torch.Tensor:
...
def mhd_all_gather_object(value: Any, context: MHDDistributedContext) -> List[Any]:
...
class MHDForwardAdapter(nn.Module):
...
def unwrap_mhd_graph(module: nn.Module) -> MHD_Graph:
    ...
    if not isinstance(candidate, MHDForwardAdapter):
        raise TypeError(f"Expected MHDForwardAdapter, found {type(candidate)!r}")
