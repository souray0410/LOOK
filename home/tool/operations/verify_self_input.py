...
def main():
    ...
    config=SimpleNamespace(correction_nodes=['joint_layer2','joint_layer3','fusion_participant_feature'],downsample_factors=[16],latent_dims=[8],max_pca_rank=512)
    outputs={};filler=NormalizedMeanFiller()
    for method in ('self_input_missing_only',):
      for pattern in ('oct_missing','cfp_missing'):
        bank,decision=fit_control(graph,train,val,device,pcas,config,pattern,method,a.output/method/pattern,source['checkpoint']['sha256'])
        again,_=fit_control(graph,train,val,device,pcas,config,pattern,method,a.output/method/pattern,source['checkpoint']['sha256'])
        assert [x.node_name for x in bank]==[x.node_name for x in again]
        evaluated=evaluate_control(graph,val,device,{pattern:bank},policy=method,filler=filler,fixed_pattern=pattern)
        assert evaluated['metrics']['macro_f1']==decision['macro_f1']
        random=evaluate_control(graph,val,device,{pattern:bank},policy=method,filler=filler,random_ratio=.4)
        evidence=representation_evidence(graph,val,device,pattern,filler,bank,method,a.output/method/pattern/'analysis')
        cost=measure_inference(graph,next(iter(val)),device,pattern,filler,bank,method,repeats=2,warmup=1)
        outputs[method+'_'+pattern]=dict(decision=decision,diagnostics=evidence,cost=cost,random_metrics=random['metrics'])
    verify_source(source)
    atomic_write_json(dict(status='passed',test_access=False,completed_at_utc=utc_now(),scope='acceptance_only; 6 train / 6 validation; CPU; two pre-fusion nodes and final participant node; both directions; one dimension; no formal metrics',source=source,outputs=outputs),a.output/'verification.json')
