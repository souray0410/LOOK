...
def main():
    ...
    parser.add_argument('--session', default='look-reviewed')
    ...
    summaries = sorted((runs / 'reviewed_look').glob('*/summary.json'), key=lambda p: p.stat().st_mtime)
    if summaries:
            ...
            for experiment_id in plan['experiment_ids']:
                ...
                if result.exists():
                    ...
                    for bank in sorted((root / 'look').glob('*/factors/x*')):
                        n = len(list((bank / 'selected').glob('*.pt')))
                        ...
                        if record.exists():
                            ...
                            last_node = rows[-1]['node'] if rows else 'none yet'
