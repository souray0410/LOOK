...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      echo "Usage: $0 [--project-root PATH] [--gpus 0,1] [--cache-workers N] [--skip-cache-warm] [--session NAME] [-- EXTRA_STEP19_ARGS]"
      exit 0
      ;;
...
if [[ "$WORKER" -eq 1 ]]; then
  ...
  if [[ "$WARM_CACHE" -eq 1 ]]; then
    ...
    "$PYTHON" "$PROJECT_ROOT/tool/operations/warm_preprocess_cache.py" \
      --project-root "$PROJECT_ROOT" --workers "$CACHE_WORKERS" 2>&1 | tee -a "$LOG_FILE"
