...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      echo "Usage: $0 [--project-root PATH] [--gpus 0,1] [--mode task-scout|baseline-selection|full-study] [--cache-workers N] [--skip-cache-warm] [--session NAME] [-- EXTRA_ARGS]"
      exit 0
      ;;
...
if [[ "$WORKER" -eq 1 ]]; then
  ...
  if [[ "$WARM_CACHE" -eq 1 && "$STUDY_MODE" != "task-scout" ]]; then
  ...
  set +e
  set -o pipefail
  if [[ "$STUDY_MODE" == "task-scout" ]]; then
    "$PYTHON" "$PROJECT_ROOT/pipeline/30_run_task_scout.py" \
      --project-root "$PROJECT_ROOT" "${RUNTIME_ARGS[@]}" \
