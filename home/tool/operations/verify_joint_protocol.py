...
def main():
    ...
    stages=runpy.run_path(str(project/'pipeline/36_run_joint_look_study.py'))['study_stages']
    candidate=json.loads(args.candidate.read_text())
    case=expand_study_grid(stages(candidate,[16],[1,2],4)[0][1],paths,gpu_devices=(0,1))[0]
    runner=ExperimentRunner(case.config,case.selection,replace(case.options,train_if_missing=False),torch.device('cuda:0'))
    if runner._backbone_id() not in candidate['winner']['backbone_ids']:
        raise RuntimeError('Smoke must use the exact reviewed backbone')
    checkpoint=runner._train_or_resume()
    before=sha256(checkpoint)
    graph,_=runner._load_frozen_graph(checkpoint)
    seed_everything(3407)
    identity=dict(protocol=PROTOCOL,implementation_sha256=implementation_sha256(project),checkpoint_sha256=before,train_limit=9,validation_limit=12,Dmax=4,factors=[16],dimensions=[1,2])
    output=paths.runs_root/'maintenance'/'joint_smoke'/stable_hash(identity)[:12]
    ...
    bank=prepare_complete_pca_bank(graph,loaders['train'],sites,[16],4,device,output/'pca',identity,output/'quarantine')
    matrices={}
    for pattern in ('oct_missing','cfp_missing'):
        selected,_=greedy_fit_look(graph,loaders['train'],loaders['validation'],pattern,sites,[16],[1,2],4,device,output/pattern,bank,primary_metric='macro_auroc_ovr')
        ...
        assert result['metrics']['macro_auroc_ovr'] >= baseline['metrics']['macro_auroc_ovr']
