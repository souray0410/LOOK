...
SESSION="look-glaucoma-baseline"
LINES=25
MODE="summary"
while [[ $# -gt 0 ]]; do
  case "$1" in
...
PREPROCESS_CACHE="$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["deployment_defaults"]["preprocess_cache_root"])' "$CONFIG")"
...
task_progress = sorted(
    (runs / "task_scout").glob("scout__*/progress.json"),
    key=lambda p: p.stat().st_mtime,
)
if task_progress:
    progress_path = task_progress[-1]
    progress = json.loads(progress_path.read_text())
    print(f"task_scout={progress_path.parent.name}")
    print(f"task_scout_status={progress.get('status')}")
    print(f"task_scout_configurations={progress.get('completed', 0)}/{progress.get('total', progress.get('configuration_count'))}")
    if progress.get("task_profile"):
        print(f"current_task={progress.get('task_profile')}")
        print(f"current_fusion={progress.get('fusion_position')}")
    leaderboard_path = progress_path.with_name("leaderboard.json")
    if leaderboard_path.is_file():
        rows = json.loads(leaderboard_path.read_text())
        print("task_scout_leaderboard:")
        for rank, row in enumerate(rows, start=1):
            print(
                f"  {rank}. task={row['task_profile']} fusion={row['fusion_position']} "
                f"eligible={row['final_task_eligible']} n_cases={row['prevalent_cases']} "
                f"auroc={row.get('macro_auroc_ovr')} macro_f1={row.get('macro_f1')} "
                f"sensitivity={row.get('sensitivity_per_class', [None, None])[1]} "
                f"specificity={row.get('specificity_per_class', [None, None])[1]}"
            )
...
if histories:
    ...
    if history:
        ...
        print("latest_training_stage=complete_modalities")
        ...
        best = max(history, key=lambda item: float(item.get('validation', {}).get('macro_auroc_ovr', -1.0)))
        print(f"best_epoch_so_far={best.get('epoch')}")
        print(f"best_auroc_so_far={best.get('validation', {}).get('macro_auroc_ovr')}")
        print(
...
if diagnostics:
    ...
    for axis, groups in summary.get("groups", {}).items():
        ordered = sorted(
            groups.items(),
            key=lambda item: -float(item[1].get("mean_macro_auroc", -1.0)),
        )
        compact = [
            {
                "value": value,
                ...
                "mean_macro_auroc": values.get("mean_macro_auroc"),
