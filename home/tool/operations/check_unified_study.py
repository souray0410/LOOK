...
def main():
    ...
    parser.add_argument('--session', default='look-unified-20260904-191807')
    ...
    summaries = sorted((runs / 'unified_study').glob('*/summary.json'), key=lambda p: p.stat().st_mtime)
    if summaries:
...
if __name__ == '__main__':
    try:
        ...
        raise SystemExit(f"CHECK_FAILED: {type(error).__name__}: {error}")
