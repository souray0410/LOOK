...
def main():
    ...
    from look_core.method_logit import freeze_direction,frozen_random,transform_logits
    spec=dict(identity_l2=1e-6,minimum_slope=1e-8,max_iterations=2000)
    bundles={p:evaluate_control(graph,val,device,filler=filler,fixed_pattern=p) for p in ('complete','oct_missing','cfp_missing')}
    params={}
    for p in ('oct_missing','cfp_missing'):
        trained=evaluate_control(graph,train,device,filler=filler,fixed_pattern=p)
        record=freeze_direction(trained,bundles[p],spec)
        assert record==freeze_direction(trained,bundles[p],spec)
        params[p]=record['selected'];outputs[p]=record
    for ratio in (.2,.4,.6,.8,1.):
        outputs[f'random_{ratio}']=frozen_random(bundles,params,ratio,3407)['metrics']
    # Exercise actual case persistence, frozen evaluation and completed-case resume.
    from look_core import method_logit
    full_spec=read_json(a.project_root/'configs/method_evidence.json')
    full_spec['acceptance_only']='six balanced train/validation participants; never formal evidence'
    proxy=SimpleNamespace(_build_loaders=lambda:({'look_train':train,'validation':val},datasets),
        _train_or_resume=lambda:None,_load_frozen_graph=lambda checkpoint:(graph,None))
    case=dict(case_id='acceptance_logit_affine',method='logit_affine',seed=3407,
        fusion_position='layer3',filling='normalized_mean')
    with patch.object(method_logit,'SuffixRunner',lambda *args,**kwargs:proxy):
        result=method_logit.run_logit_case(source,case,full_spec,a.output/'logit_case',device,())
        resumed=method_logit.run_logit_case(source,case,full_spec,a.output/'logit_case',device,())
        assert result==resumed and result['test_access'] is False and len(result['validation'])==8
    outputs['persisted_logit_case']=file_record(a.output/'logit_case'/'validation_result.json')
    verify_source(source)
    atomic_write_json(dict(status='passed',test_access=False,completed_at_utc=utc_now(),scope='acceptance_only; 6 train / 6 validation; CPU; bias-only two final nodes one dimension; output control both directions all ratios; not formal metrics',
