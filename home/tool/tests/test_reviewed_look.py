...
def test_reviewed_queue_is_two_quick_then_nine_full_cases():
    ...
    assert stages[0][0] == 'quick_zero_mean'
    ...
    assert stages[0][1].look_profiles[0]['downsample_factors'] == [16]
    for _, grid in stages[1:]:
        grid.validate()
...
def test_dmax_and_search_list_are_independent():
    api = runpy.run_path(str(Path(__file__).resolve().parents[2] / 'pipeline/34_run_reviewed_look_study.py'))
    candidates = api['latent_candidates']
    assert candidates(None, 32, 128, 32, 512) == [32, 64, 96, 128]
    assert candidates([8, 32, 128], None, None, None, 512) == [8, 32, 128]
    with pytest.raises(ValueError):
        candidates([32], 32, 128, 32, 512)
    with pytest.raises(ValueError):
        candidates([1024], None, None, None, 512)
