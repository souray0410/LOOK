...
def test_test_access_requires_explicit_frozen_confirmation(tmp_path):
    with pytest.raises(ValueError, match="CONFIGURATION_FROZEN"):
        ExperimentRunner(
            _config(tmp_path),
            ExperimentSelection(),
            PipelineOptions(allow_test=True),
            torch.device("cpu"),
        )
