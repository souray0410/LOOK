...
def test_calibration_grid_has_six_conventional_profiles(tmp_path):
    ...
    assert {case.config.label_smoothing for case in cases} == {0.0}
...
def test_approved_candidate_requires_explicit_note(tmp_path):
    ...
    candidate.write_text(json.dumps({
        ...
        "selection_id": "abc", "protocol": "ukb_glaucoma_binary_baseline_selection",
...
def test_frozen_baseline_defines_formal_look_grid(tmp_path):
    ...
    manifest.write_text(json.dumps({
        "status": "frozen", "protocol": "ukb_glaucoma_binary_baseline_selection",
        "winner": {"fusion_position": "layer3", "seeds": [3407, 3408, 3409]},
        "classifier_profile": baseline_search_grid().classifier_profiles[0],
        "artifacts": build_file_manifest([checkpoint]),
    }), encoding="utf-8")
...
def test_notebook_has_one_documented_configuration_cell():
    notebook = json.loads(
        (Path(__file__).resolve().parents[2] / "pipeline/27_UKB_LOOK_Glaucoma_ResNet50_MHD.ipynb")
