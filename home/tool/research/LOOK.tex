...
\title[LOOK in MHD V3]{LOOK in MHD Framework V3}
...
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture,overlay]
    ...
    \node[anchor=north west,rounded corners=2pt,fill=LookTeal,text=white,
      ...
      ([xshift=11mm,yshift=-47mm]current page.north west) {\small PhD foundation: MHD V3};
...
\begin{frame}{Storyline for this discussion}
  ...
  \begin{exampleblock}{One sentence}
    MHD V3 exposes the internal computation graph; LOOK uses that access to estimate and inject a
...
\begin{frame}{MHD V3: code-level contract used by LOOK}
  \small
...
\begin{frame}{Preliminary validation: a positive signal, not a final claim}
  \begin{columns}[T,onlytextwidth]
    ...
    \column{0.40\textwidth}
      \resizebox{\linewidth}{!}{\begin{tikzpicture}[x=1cm,y=34cm,font=\scriptsize]
...
\begin{frame}{Comparison landscape: LOOK is a correction layer, not a closed pipeline}
  \centering
  \resizebox{0.97\textwidth}{!}{\begin{tikzpicture}[node distance=4mm,font=\small]
...
\begin{frame}{Interpreting the correction matrix can become a scientific contribution}
  \begin{columns}[T,onlytextwidth]
    ...
    \column{0.47\textwidth}
      \resizebox{0.96\linewidth}{!}{\begin{tikzpicture}[font=\scriptsize]
...
\begin{frame}{Roadmap: data, models, and system scale}
  \begin{tikzpicture}[node distance=2.5mm,font=\footnotesize]
    \node[enc,text width=2.85cm] (now) {Now\\BraTS, one pattern\\3D U-Net, single GPU};
    \node[core,text width=2.85cm,right=of now] (near) {Near term\\full ablations\\larger-memory single GPU};
    \node[dec,text width=2.85cm,right=of near] (mid) {Mid term\\multicenter cardiac MRI\\2D / 2.5D / larger models};
    \node[stage,text width=2.85cm,right=of mid] (long) {Long term\\multi-GPU MHD V3\\distributed graph execution};
    \draw[flow] (now) -- (near);
    \draw[flow] (near) -- (mid);
    \draw[flow] (mid) -- (long);
  \end{tikzpicture}
  ...
  \begin{alertblock}{Engineering constraint}
    MHD V3 is currently practical on one GPU. A larger-memory GPU is the shortest path to the next
    experiments; multi-GPU support should follow after the method and graph semantics are stable.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Decisions requested from the supervisor}
  ...
  \begin{tabularx}{\textwidth}{>{\bfseries\color{LookNavy}}p{2.7cm} Y}
    ...
    System roadmap & When should MHD V3 multi-GPU support become a research deliverable rather than infrastructure work? \\
    \bottomrule
  \end{tabularx}
  \vspace{1.5mm}
...
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture,overlay]
    ...
    \node[text=LookPaleBlue,font=\large,align=center] at ([yshift=-9mm]current page.center)
      {MHD V3 as the intervention framework\\LOOK as the first linear correction study};
  \end{tikzpicture}
\end{frame}
