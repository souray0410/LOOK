...
\begin{frame}{Roadmap: data, models, and system scale}
  \begin{tikzpicture}[node distance=2.5mm,font=\footnotesize]
    ...
    \node[stage,text width=2.85cm,right=of mid] (long) {Long term\\multi-node MHD V3\\distributed graph execution};
    \draw[flow] (now) -- (near);
    \draw[flow] (near) -- (mid);
    \draw[flow] (mid) -- (long);
  \end{tikzpicture}
  ...
  \begin{alertblock}{Engineering constraint}
    MHD V3 graph logic remains unchanged; execution adapters now support one- or two-GPU runs.
