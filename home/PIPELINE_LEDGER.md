...
| 15 | remote | Run unit and governance tests | pytest result | Single visible GPU; no data mutation |
| 16 | remote | Smoke-test MHD fusion topologies | smoke result | Seven fusion positions; one GPU |
| 17 | remote | Tiny dual-filling integration run | `runs/smoke/` | mean and paired cGAN; checkpoint resume |
| 18 | remote | Interactive complete study entry | deterministic experiment runs | One config cell; list grid; stage resume |
| 19 | remote | Execute formal study grid | sweep plan/progress | Dry-run; sequential cases; sealed test gate |
...
- Compute: GPU, workers, micro/effective batches and AMP.
